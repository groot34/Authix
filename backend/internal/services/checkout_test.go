...
func TestCheckoutService_Validation(t *testing.T) {
	...
	cases := []struct {
		...
		{"email without domain suffix", set(func(s *CheckoutSubmission) { s.Email = "user@localhost" }), ErrInvalidEmail},
		{"email with multiple at signs", set(func(s *CheckoutSubmission) { s.Email = "user@@example.com" }), ErrInvalidEmail},
		{"empty phone", set(func(s *CheckoutSubmission) { s.Phone = "  " }), ErrInvalidPhone},
		{"phone with letters", set(func(s *CheckoutSubmission) { s.Phone = "+1 555 ABC 0100" }), ErrInvalidPhone},
		{"phone with emoji", set(func(s *CheckoutSubmission) { s.Phone = "+1 555 0100 😀" }), ErrInvalidPhone},
		{"phone too short", set(func(s *CheckoutSubmission) { s.Phone = "123456" }), ErrInvalidPhone},
		...
		{"city with digits", set(func(s *CheckoutSubmission) { s.ShippingCity = "Town 42" }), ErrInvalidShippingCity},
		{"city with emoji", set(func(s *CheckoutSubmission) { s.ShippingCity = "Town 😀" }), ErrInvalidShippingCity},
		{"empty postal", set(func(s *CheckoutSubmission) { s.ShippingPostalCode = "" }), ErrInvalidShippingPostal},
		{"postal with symbols", set(func(s *CheckoutSubmission) { s.ShippingPostalCode = "12😀45" }), ErrInvalidShippingPostal},
		{"empty region", set(func(s *CheckoutSubmission) { s.ShippingRegion = "" }), ErrInvalidShippingRegion},
		{"region with digits", set(func(s *CheckoutSubmission) { s.ShippingRegion = "Region 7" }), ErrInvalidShippingRegion},
